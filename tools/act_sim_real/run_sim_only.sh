#!/usr/bin/env bash
# Reproducible Task_000458 sim-only ACT workflow. Run from any directory.
set -euo pipefail
...
DATASET_ROOT="$WORKSPACE/dataset/robotis/task_000458_peanut_mix_sim_only_act_v30"
DATASET_REPO_ID="robotis/task_000458_peanut_mix_sim_only_act_v30"
...
ACT_JOB_NAME="${ACT_JOB_NAME:-task_000458_peanut_mix_sim_only_act}"
ACT_OUTPUT_DIR="${ACT_OUTPUT_DIR:-/workspace/model/lerobot/${ACT_JOB_NAME}}"
ACT_LOG_FILE="${ACT_LOG_FILE:-$WORKSPACE/model/lerobot/logs/${ACT_JOB_NAME}.log}"
...
CONTAINER_DATASET_ROOT="$CONTAINER_WORKSPACE/dataset/robotis/task_000458_peanut_mix_sim_only_act_v30"

usage() {
  cat <<'EOF'
Usage: tools/act_sim_real/run_sim_only.sh <sync-real|validate|build-dataset|train|all>
...
build-dataset Convert all sim-only episodes into one 224x224, 10 Hz LeRobot v3 dataset.
...
build_dataset() {
  ...
  docker exec "$MAIN_CONTAINER" env CYCLO_PREPARED_EPISODE_CACHE_DISABLE=1 CYCLO_V30_SOURCE_AGGREGATE_CACHE_DISABLE=1 CYCLO_V30_DATA_AGGREGATE_CACHE_DISABLE=1 python3 "$CONTAINER_REPO/tools/act_sim_real/convert_sim_real.py" \
    ...
    --fps 10 --image-height "$CONVERSION_IMAGE_HEIGHT" --image-width "$CONVERSION_IMAGE_WIDTH"
}
